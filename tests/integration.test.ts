...
test.describe('Integration Tests', () => {
  ...
  test('should have diverse party representation', async () => {
    ...
    const majorParties = ['自民', '立憲', '維新', '公明', '国民', '共産'];
  ...
  test('should extract election counts for members', async () => {
    ...
    const membersWithSenateCount = membersWithElectionCount.filter(
      (member) => typeof member.electionCount === 'object' && member.electionCount?.senate
    );
    ...
    if (membersWithSenateCount.length > 0) {
      ...
      if (memberWithSenate && typeof memberWithSenate.electionCount === 'object') {
  ...
  test('data consistency validation', async () => {
    ...
    for (const member of singleSeatMembers.slice(0, 20)) {
      expect(member.election.prefecture).toBeDefined();
      expect(PREFECTURES).toContain(member.election.prefecture as string);
    ...
    if (proportionalMembers.length > 0) {
      for (const member of proportionalMembers.slice(0, 10)) {
        // Check first 10
        ...
        const hasValidArea =
          validPropAreas.some((area) => member.election.area?.includes(area)) ||
          (member.election.area?.includes('ブロック') ?? false);
  ...
  test('should handle real-world data edge cases', async () => {
    ...
    const issues = {
      ...
      unknownDistricts: result.members.filter(
        (m) => m.election.system === 'single-seat' && m.election.prefecture === '不明'
      ),
      invalidElectionCounts: result.members.filter((m) => {
        ...
        if (typeof m.electionCount === 'number') {
          ...
          return (
            m.electionCount.house <= 0 ||
            m.electionCount.house > 25 ||
