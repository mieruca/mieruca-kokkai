...
export class DietMemberScraper {
  ...
  async scrapeHouseOfRepresentatives(): Promise<ScrapeResult> {
    ...
    try {
      await page.goto(
        'https://www.shugiin.go.jp/internet/itdb_shitsumei.nsf/html/shitsumon/menu_m.htm'
      );
  ...
  private async scrapeMemberDetail(page: Page, url: string): Promise<DietMember | null> {
    try {
      ...
      const name = await page
        .$eval('h1, .member-name, .name', (el) => el.textContent?.trim() || '')
        .catch(() => '');

      if (!name) return null;

      const party = await page
        .$eval('[class*="party"], [class*="政党"]', (el) => el.textContent?.trim() || '')
        .catch(() => '不明');

      const prefecture = await page
        .$eval(
          '[class*="prefecture"], [class*="選挙区"], [class*="都道府県"]',
        ...
        for (const row of rows) {
          ...
          if (cells.length >= 3) {
            ...
            if (name && name !== '氏名' && name !== '') {
              ...
              const prefectureCell = cells[2];

              members.push({
                name,
                party: partyCell?.textContent?.trim() || '不明',
                prefecture: prefectureCell?.textContent?.trim() || '不明',
